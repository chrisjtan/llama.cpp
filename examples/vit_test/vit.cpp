...
static bool encode_image_with_clip(clip_ctx * ctx_clip, int n_threads, const clip_image_u8 * img, float * image_embd, int * n_img_pos) {
    ...
    if (strcmp(mm_patch_merge_type, "spatial_unpad") != 0) {
        ...
        *n_img_pos = clip_n_patches(ctx_clip);  // 96
        // std::cout << "n img pos: \n\n" << *n_img_pos << std::endl;
        bool encoded = clip_image_encode(ctx_clip, n_threads, &img_res_v.data[0], image_embd, load_image_size); // 576 x 4096
        delete[] img_res_v.data;
        if (!encoded) {
...
bool llava_image_embed_make_with_clip_img(clip_ctx * ctx_clip, int n_threads, const clip_image_u8 * img, float ** image_embd_out, int * n_img_pos_out) {
    float * image_embd = (float *)malloc(clip_embd_nbytes(ctx_clip)*6); // Note: This could be changed to be dynamic

    // size_t num_elements = clip_embd_nbytes(ctx_clip) * 6 / sizeof(float);
    // printf("Number of float elements in image_embd: \n\n%zu\n", num_elements);
...
std::vector<std::vector<struct llava_image_embed *>> llava_image_embed_make_with_bytes_slice(struct clip_ctx * ctx_clip, int n_threads, const unsigned char * image_bytes, int image_bytes_length) {
    ...
    // std::cout << "Image dimensions: " << img->nx << "x" << img->ny << std::endl;
    
    // // Print the buffer size
    // std::cout << "Buffer size: " << img->buf.size() << std::endl;

    // // Optionally print the first few bytes of the buffer (e.g., first 100 bytes)
    // std::cout << "Buffer contents (first 100 bytes or fewer): ";
    // for (size_t i = 0; i < std::min(img->buf.size(), size_t(100)); ++i) {
    //     std::cout << std::hex << static_cast<int>(img->buf[i]) << " ";
    // }
    ...
    // std::cout <<"start checking image slices !!\n\n\n";
    // // Number of rows
    // size_t num_rows = imgs.size();
    // std::cout << "Number of rows: " << num_rows << std::endl;
    // // Number of elements (slices) in each row
    // for (size_t i = 0; i < num_rows; ++i) {
    //     size_t num_slices = imgs[i].size();
    //     std::cout << "Row " << i << " has " << num_slices << " slices" << std::endl;
    //     // Iterate through each slice in the row
    //     for (size_t j = 0; j < num_slices; ++j) {
    //         clip_image_u8* slice = imgs[i][j];
    //         std::cout << "  Slice " << j << " dimensions: " << slice->nx << "x" << slice->ny << std::endl;
    //     }
    // }
    // std::cout <<"end checking image slices !!\n\n\n";

    //     Number of rows: 3
    //     Row 0 has 1 slices
    //          Slice 0 dimensions: 392x518
    //      Row 1 has 1 slices
    //          Slice 0 dimensions: 546x364
    //      Row 2 has 1 slices
    //          Slice 0 dimensions: 546x364
...
std::vector<std::vector<struct llava_image_embed *>> llava_image_embed_make_with_filename_slice(struct clip_ctx * ctx_clip, int n_threads, const char * image_path) {
    ...
    // // Print the image_bytes_length
    // std::cout << "Image bytes length: " << image_bytes_length << std::endl;

    // // Optionally print the first few bytes of the image
    // std::cout << "Image bytes (first 100 bytes or fewer): ";
    // for (long i = 0; i < std::min(image_bytes_length, 100L); ++i) {
    //     std::cout << std::hex << static_cast<int>(image_bytes[i]) << " ";
    // }
    // std::cout << std::dec << std::endl; // Reset to decimal output after printing in hex
    ...
    // embeds outer: number of slices. inner: 1, embedding is float number with size dim * patches.
    // std::cout << "Outer vector size: \n\n" << embeds.size() << std::endl;
    // for (size_t i = 0; i < embeds.size(); ++i) {
    //     std::cout << "Size of inner vector \n\n" << i << ": " << embeds[i].size() << std::endl;
    // }
    free(image_bytes);
    return embeds;
}
