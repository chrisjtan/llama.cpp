...
int main(int argc, char ** argv) {
    ...
    // Check params
    std::cout<< "mmproj path: " << params.mmproj << std::endl;
    
    // only generate image tokens
    for (auto & image : params.image) {
        // for each image, encode and project to embedding
        auto image_embed_slices = minicpmv_image_embed(&params, image);
        if (!image_embed_slices[0][0]) {
    ...
    // for (auto & image : params.image) {
    //     int n_past = 0;
    //     auto ctx_llava = minicpmv_init(&params, image, n_past);  // This function load the parameters from the gguf, and return embedding after projection
