...
// xgen-mm
// #degine google siglip multiheadattentionpoolinghead
#define TN_HEAD_PROB "%s.head.probe"
#define TN_HEAD_ATTN_Q "%s.head.attention.q_%s"
#define TN_HEAD_ATTN_K "%s.head.attention.k_%s"
#define TN_HEAD_ATTN_V "%s.head.attention.v_%s"
#define TN_HEAD_ATTN_OUT "%s.head.attention.out.%s"
#define TN_HEAD_LN "%s.head.ln.%s"
#define TN_HEAD_FFN_DOWN "%s.head.ffn_down.%s"
#define TN_HEAD_FFN_UP "%s.head.ffn_up.%s"
...
struct clip_vision_model {
    ...
    // google siglip has PoolingHead
    struct ggml_tensor * head_probe;
    struct ggml_tensor * head_attn_q_w;
    struct ggml_tensor * head_attn_q_b;
    struct ggml_tensor * head_attn_k_w;
    struct ggml_tensor * head_attn_k_b;
    struct ggml_tensor * head_attn_v_w;
    struct ggml_tensor * head_attn_v_b;
    struct ggml_tensor * head_attn_out_w;
    struct ggml_tensor * head_attn_out_b;
    struct ggml_tensor * head_ln_w;
    struct ggml_tensor * head_ln_b;
    struct ggml_tensor * head_ff_i_w;
    struct ggml_tensor * head_ff_i_b;
    struct ggml_tensor * head_ff_o_w;
    struct ggml_tensor * head_ff_o_b;
...
static ggml_cgraph * clip_image_build_graph(clip_ctx * ctx, const clip_image_f32_batch * imgs, std::pair<int, int> load_image_size = {448, 448}) {
    ...
    // debug: hard code to change image_size to 384x384 for quick testing
    load_image_size.first = 384;
    load_image_size.second = 384;
    const int image_size_width     = load_image_size.first;
    const int image_size_height    = load_image_size.second;
    ...
    ggml_tensor * ans;  // use this tensor to test output
    ...
    // // add xgen-mm pool head layers here
    // cross attention layer
    // add dimension
    struct ggml_tensor * probe = ggml_repeat(ctx0, model.head_probe, embeddings);
    // struct ggml_tensor * Q =
    //     ggml_add(ctx0, ggml_mul_mat(ctx0, model.head_attn_q_w, model.head_probe), model.head_attn_q_b);
    struct ggml_tensor * Q =
        ggml_add(ctx0, ggml_mul_mat(ctx0, model.head_attn_q_w, probe), model.head_attn_q_b);
    Q = ggml_scale_inplace(ctx0, Q, 1.0f / sqrt((float)d_head));
    Q = ggml_reshape_4d(ctx0, Q, d_head, n_head, num_positions, batch_size);
    Q = ggml_cont(ctx0, ggml_permute(ctx0, Q, 0, 2, 1, 3));
    Q = ggml_reshape_3d(ctx0, Q, d_head, num_positions, n_head * batch_size);
    ...
    struct ggml_tensor * K =
        ggml_add(ctx0, ggml_mul_mat(ctx0, model.head_attn_k_w, embeddings), model.head_attn_k_b);
    K = ggml_reshape_4d(ctx0, K, d_head, n_head, num_positions, batch_size);
    K = ggml_cont(ctx0, ggml_permute(ctx0, K, 0, 2, 1, 3));
    K = ggml_reshape_3d(ctx0, K, d_head, num_positions, n_head * batch_size);

    struct ggml_tensor * V =
        ggml_add(ctx0, ggml_mul_mat(ctx0, model.head_attn_v_w, embeddings), model.head_attn_v_b);
    ...
    embeddings = ggml_cont_3d(ctx0, KQV, hidden_size, num_positions, batch_size);
    // attention output
    embeddings = ggml_add(ctx0, ggml_mul_mat(ctx0, model.head_attn_out_w, embeddings), model.head_attn_out_b);
    ...
    {
        embeddings = ggml_norm(ctx0, embeddings, eps);
        embeddings = ggml_add(ctx0, ggml_mul(ctx0, embeddings, model.head_ln_w),
                        model.head_ln_b);
    }
        //mlp
    {
        embeddings = ggml_mul_mat(ctx0, model.head_ff_i_w, embeddings);
        embeddings = ggml_add(ctx0, embeddings, model.head_ff_i_b);
        ...
        embeddings = ggml_mul_mat(ctx0, model.head_ff_o_w, embeddings);
        embeddings = ggml_add(ctx0, embeddings, model.head_ff_o_b);
    }
        // add residual
    ...
    // struct ggml_tensor * W_q = ggml_view_2d(ctx0, model.head_attn_in_w, embed_dim, /*cols=*/embed_dim, /*offset=*/0);
    // // struct ggml_tensor * qkv = ggml_mul_mat(ctx0, model.);
    // struct ggml_tensor * Q = ggml_mul_mat(ctx0, model.head_attn_in_w, model.head_probe);
    // Q = ggml_add(ctx0, Q, model.head_attn_in_b);

    // struct ggml_tensor * KV =
    //     ggml_add(ctx0, ggml_mul_mat(ctx0, model.head_attn_in_w, embeddings), model.head_attn_in_b);
...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity = 1, std::pair<int, int> load_image_size = {448, 448}) {
    ...
    if (verbosity >= 1) {
        ...
        // // debug
        // std::cout << "check n tensor value \n\n" << n_tensors << std::endl;
        // const char * name = gguf_get_tensor_name(ctx, 0);
        // std::cout << "check tensor name \n\n" << name << std::endl;
        // // debug end
        ...
        __func__, n_kv, n_tensors, fname);
    {
        ...
        for (int i = 0; i < n_tensors; i++) {
            enum ggml_type type = gguf_get_tensor_type(ctx, i);  // GGML_TYPE_F32
            n_type[type]++;
        }
        ...
        for (int i = 0; i < n_kv; i++) {
            ...
            // std::cout << "\n\nKey:    " << name << "    key end \n"<<std::endl;
            // std::cout << "Value:    " << value << "    value end \n\n"<<std::endl;
    ...
    if (new_clip->has_vision_encoder) {
        ...
        std::cout << "Test visual encoder layers: \n" << hparams.n_layer << std::endl;
        if (verbosity >= 2) {
        ...
        try {
            ...
            // google siglip mutiheadattgentionpoolinghead
            vision_model.head_probe           = get_tensor(new_clip->ctx_data, format(TN_HEAD_PROB, "v"));
            // vision_model.head_attn_in_w           = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_IN, "v", "weight"));
            // vision_model.head_attn_in_b           = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_IN, "v", "bias"));
            vision_model.head_attn_q_w            = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_Q, "v", "weight"));
            vision_model.head_attn_q_b            = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_Q, "v", "bias"));
            vision_model.head_attn_k_w            = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_K, "v", "weight"));
            vision_model.head_attn_k_b            = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_K, "v", "bias"));
            vision_model.head_attn_v_w            = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_V, "v", "weight"));
            vision_model.head_attn_v_b            = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_V, "v", "bias"));
            vision_model.head_attn_out_w           = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_OUT, "v", "weight"));
            vision_model.head_attn_out_b           = get_tensor(new_clip->ctx_data, format(TN_HEAD_ATTN_OUT, "v", "bias"));
            vision_model.head_ln_w                 = get_tensor(new_clip->ctx_data, format(TN_HEAD_LN, "v", "weight"));
            vision_model.head_ln_b                 = get_tensor(new_clip->ctx_data, format(TN_HEAD_LN, "v", "bias"));
            vision_model.head_ff_i_w              = get_tensor(new_clip->ctx_data, format(TN_HEAD_FFN_DOWN, "v", "weight"));
            vision_model.head_ff_i_b              = get_tensor(new_clip->ctx_data, format(TN_HEAD_FFN_DOWN, "v", "bias"));
            vision_model.head_ff_o_w              = get_tensor(new_clip->ctx_data, format(TN_HEAD_FFN_UP, "v", "weight"));
            vision_model.head_ff_o_b              = get_tensor(new_clip->ctx_data, format(TN_HEAD_FFN_UP, "v", "bias"));
        } catch(const std::exception& e) {
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs, float * vec, std::pair<int, int> load_image_size = {448, 448}) {
    ...
    // // check some other information
    // std::cout<< "has_visual_encoder: " << ctx->has_vision_encoder<<"\n\n\n\n"<<std::endl;
    // std::cout<< "has_projecter: " << ctx->has_minicpmv_projector<<"\n\n\n\n"<<std::endl;
    // std::cout<< "has_test_encoder: " << ctx->has_text_encoder<<"\n\n\n\n"<<std::endl;

    if (ctx->has_minicpmv_projector) {
        GGML_ASSERT(batch_size == 1); // TODO: support multiple images
    }
    // For a dummy image patch
    load_image_size.first = 384;
    load_image_size.second = 384;
    // debug end
...
\
    {
        ...
        // use a dummy image for now
        const int nx = 384;  // width
        const int ny = 384;  // height
        const int n = nx * ny;
        ...
        //             for (int y = 0; y < ny; y++) {
        //                 for (int x = 0; x < nx; x++) {
        //                     data[(b * 3 * n) + k * n + y * nx + x] = imgs->data[b].buf[3 * (y * nx + x) + k];  // channel first
        //                 }
        //             }
        //         }
        //     }
        ...
        // // Print the raw data
        // size_t data_size = ggml_nbytes(inp_raw) / sizeof(float);
    ...
    //     int n = 0;
    //     float t = 0;
    //     // std::cout << "num positions:\n\n" << num_positions<<std::endl;
...
// debug: pos_embedding not needed for testting VIT
//     {
//         struct ggml_tensor * pos_embed = ggml_graph_get_tensor(gf, "pos_embed");
//         std::cout<<"pos_embded size: \n\n"<<ggml_nbytes(pos_embed)<<std::endl;
//         int pos_w = image_size_width/patch_size;
//         int pos_h = image_size_height/patch_size;
//         int embed_dim = 4096;
...
// debug end
...
#endif
    // std::cout << "I think at least should come here:  \n\n\n" << std::endl;
    ggml_backend_graph_compute(ctx->backend, gf);
    // std::cout << "May be not computed!:  \n\n\n" << std::endl;
    ...
    ggml_graph_dump_dot(gf, NULL, "vit.dot");
